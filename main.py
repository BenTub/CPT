...
def draw_car_health(surf, x, y, pct):
    ...
    if pct > 0.6:
        ...
        col = RED
    #draws this rectange to the screen and fills it with the correct color
    pg.draw.rect(surf, col, fill_rect)
    pg.draw.rect(surf, WHITE, outline_rect, 2)


def collide_with_walls(sprite, group, dir):
    #using cordinates at the center of each sprite rectange to detects if they collide or overlap
    #checks the x cordinate
    if dir == 'x':
...
class Car(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        #creates rectangle variable that is placed over the player which is used to determine if it collides with anything
    ...
    def get_keys(self):
        ...
        if keys[pg.K_RIGHT] or keys[pg.K_d]:
            self.rot_speed = -CAR_ROT_SPEED
        #checks which level the game is using global variable then ajusts the speed to make the car move faster
        ...
        if keys[pg.K_SPACE]:
            #assiging variabe now to make sure bullets dont loop endlessly
...
class Mob(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        #assiging the mob sprite to the all sprites group
        ...
        self.rect.center = self.pos
        self.rot = 0
        #calling the global variable level to ajust the mobs health and speed as the levels progress
        global level
        if level == 1:
    ...
    def update(self):
        ...
        self.avoid_mobs()
        #mob movment using vectors to make it move freely on its own
        self.acc.scale_to_length(self.speed)
        self.acc += self.vel * -1
        self.vel += self.acc * self.game.dt
        self.pos += self.vel * self.game.dt + 0.5 * self.acc * self.game.dt ** 2
        self.hit_rect.centerx = self.pos.x
        #checks so the mobs dont run through the walls and stay on the roads
        collide_with_walls(self, self.game.walls, 'x')
        self.hit_rect.centery = self.pos.y
        collide_with_walls(self, self.game.walls, 'y')
        self.rect.center = self.hit_rect.center
        #updates when the mobs health equels 0 then it disappear using the kill variable
        if self.health <= 0:
            self.kill()
    ...
    def draw_health(self):
        ...
        if self.health > 60:
            ...
            col = RED
        #addition if statment that checks if it is level threee than if so it adds more health to the bar
        if level == 3:
...
class Wall(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        #calling wall image and setting rectanges around it
...
class Obstacle(pg.sprite.Sprite):
    ...
    def __init__(self, game, x, y, w, h):
        ...
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        #using a rectange to make a barrier that is placed around the walls
...
class Map:
    def __init__(self, filename):
        ...
        #assigning dimesions to the map
...
class TiledMap:
    ...
    def render(self, surface):
        ...
        #making sure everything is layer properly
...
class Camera:
    ...
    def update(self, target):
        #the x and y cordinates of the player
...
class Game:
    ...
    def new(self):
        ...
        #using the global level varibale to determine what map to load in
        if level == 1:
            self.map_img = self.map.make_map()
            self.map.rect = self.map_img.get_rect()
            for tile_object in self.map.tmxdata.objects:
                obj_center = vec(tile_object.x + tile_object.width / 2, tile_object.y + tile_object.height / 2)
                #series of if statments that reads the Tiled map and places the Car, Mobs, Items or walls
                #ex if there is an object in the map with the name player it calls the car function to be assigned to that object
    ...
    def update(self):
        ...
        global level
        #game over when the length of passengers equels zero including the destination
        if len(self.items) == 0:
            self.playing = False
        #mob hits player
        ...
        for hit in hits:
            #subtracts the bullet damage from the zombies depenidng on the level
            if level == 3:
    ...
    def show_go_screen(self):
        ...
        if not self.over:
            if level == 3:
                ...
                pg.display.flip()
                'call wait for key function'
                #calls wait for key 2 which also adds one to level 
                self.wait_for_key2()
        else:
